...
namespace Core {
	...
	public:
		...
		void DrawCard() {
			...
			DrawText(m_PreviewText, m_Position.x + MeasureText(m_Button->GetText(), FONT_SIZE) / 2, m_Position.y, FONT_SIZE, BLACK);
			DrawText(m_Button->GetText(), m_Position.x + (m_Button->GetTexture().width / 2 - MeasureText(m_Button->GetText(), FONT_SIZE) / 2), m_Position.y + m_Button->GetTexture().height, FONT_SIZE, WHITE);
